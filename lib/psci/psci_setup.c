...
static DEFINE_PER_CPU(cpu_context_t, psci_ns_context);
					...
					unsigned char level)
{
	if (level > PSCI_CPU_PWR_LVL) {
		...
		FOR_CPU(psci_cpu_pd_nodes, node_idx).parent_node = parent_idx;

		/* Initialize with an invalid mpidr */
		FOR_CPU(psci_cpu_pd_nodes, node_idx).mpidr = PSCI_INVALID_MPIDR;

		svc_cpu_data =
			&(FOR_CPU_PTR(percpu_data, node_idx)->psci_svc_cpu_data);
		...
		cm_set_context_by_index(node_idx,
					(void *) FOR_CPU_PTR(psci_ns_context,
					node_idx),
					NON_SECURE);
	}
}
...
int __init psci_setup(const psci_lib_args_t *lib_args)
{
	...
	GET_THIS_CPU(psci_cpu_pd_nodes).mpidr =
